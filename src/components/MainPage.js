...
const Main = ({ isLoggedIn }) => {
  ...
  const filterProfiles = (e) => {
    ...
    console.log(
      "🚀 ~ file: MainPage.js:32 ~ filterProfiles ~ checkAgainst:",
      checkAgainst
    );
    if (checkAgainst === "default") {
      ...
      const filteredUsers = data.users.filter(
        (user) => user[`${check}`] === checkAgainst
      );

      console.log(
        "🚀 ~ file: MainPage.js:39 ~ filterProfiles ~ filteredUsers:",
        filteredUsers
      );
  ...
  return (
    <>
      <label for="profiles" style={{ marginLeft: "25%" }}>
        Filter Profiles :{" "}
      </label>
      <select
        onChange={(e) => {
      ...
      <input
        ...
        id="name"
        style={{ marginLeft: "10px", background: "" }}
